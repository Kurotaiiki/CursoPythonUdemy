...
class Worker(QtCore.QRunnable):
    ...
    def run(self):
        ...
        try:
           ...
           self.signals.error.emit(self.url,doc)
        else:
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.pushButton.clicked.connect(self.scrapearWeb)
        self.threadPool = QtCore.QThreadPool()
        print(f'Multithreading con maximo {self.threadPool.maxThreadCount()} hilos')
    ...
    def scrapearWeb(self):
        url=self.url.text()
        print(f'{horaISO()} (Rec) {url}')
        time.sleep(5)
        try:
           doc = pq(url=url)
        except Exception as error:
    ...
    def scrapearWebConcurrente(self):
        url=self.url.text()
        print(f'{horaISO()} (Rec) {url}')
        #logica de concurrencia
